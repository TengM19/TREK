...
Attachments, PDF import and plugin execution are unavailable. Realtime collaboration uses the Durable Object WebSocket transport with the original trip-room event contract. Upload writes fail explicitly; multipart uploads, backup/restore and storage reconfiguration return 501. Plugins are disabled with `TREK_PLUGINS_ENABLED=false`; child-process isolation is not replaced with in-process execution. Cloudflare Cron now runs the durable cleanup task (expired idempotency keys, WebAuthn challenges and registration invites) at 03:00 UTC and the native trip/todo reminder jobs at 09:00 UTC. The reminder service retains the original preference, recipient and in-app notification rules; the remaining upstream jobs still need individual migration. Rotating log files are replaced by console logging. Unsupported admin secret rotation fails with an instruction to rotate the Cloudflare secret.
