...
The current gateway targets `https://trek-cloudflare-demo.teng-m95.workers.dev`. Changing backend ownership or hostname requires reviewing the explicit target and session-forwarding boundary. General API bearer tokens, OIDC/passkeys, live collaboration and non-core upstream functionality have not been accepted through this gateway. The backend's Cloudflare Cron cleanup runs independently; Sites does not invoke scheduled jobs itself.
